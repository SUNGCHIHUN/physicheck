...
jQuery(function ($) {
	...
	// 회원 등록
	function addMember() {
		window.location='/admin/memberRegistList';
	}
	...
	function deleteMember() {
		...
		// 하나라도 선택한 경우
			...
			if(confirm(selected + '명의 회원을 삭제하시겠습니까?')) {
				window.location='/admin/memberDeleteAction?memberArr=' + memberArr + '&pageNum=${paging.currentPage}&${_csrf.parameterName}=${_csrf.token}';
			};	
		}
	...
	function searchMember() {
		...
		window.location='/admin/memberList?keyword=' + keyword;
	}
					...
					<td align="left">	
						<div class="input-group sm-3">
						  ...
						  <div class="input-group-append">
						  	<input type="button" name="button" id="search" class="btn btn-primary" value="검색" onclick="searchMember()">
						  </div>
						</div>
					</td>
					<td align="right">
						<input type="button" name="button" id="insert" class="btn btn-success" value="요청목록" onclick="addMember()">
						<input type="button" name="button" id="delete" class="btn btn-danger" value="삭제" onclick="deleteMember()">
					</td>
				</tr>
			</table>
		</div>
					...
					<c:forEach var="m" items="${mlist}">
						<tr>
							<td align="center" id="td_info">
								<input type = "checkbox" id="m_checkbox" name="m_checkbox" value="${m.memberNo}" <c:if test="${m.outDate != null}">disabled</c:if>>
							</td>
							...
							<td align="center">
								<c:if test="${m.outDate == null}">-</c:if>
								<c:if test="${m.outDate != null}"><span style="color: red;">${m.outDate}</span></c:if>
							</td>
							<td align="center">${m.checks}</td>
						</tr>
					</c:forEach>
					<tr>
						<td align="center" colspan="11">
							...
							<c:if test="${paging.startPage > 10}">
							    <a href="/admin/memberList?pageNum=${paging.prev}">[이전]</a>
							</c:if>
							...
							<c:if test="${paging.endPage < paging.pageCount}">
							    <a href="/admin/memberList?pageNum=${paging.next}">[다음]</a>
							</c:if>
						</td>
    			  </tr>
			</table>

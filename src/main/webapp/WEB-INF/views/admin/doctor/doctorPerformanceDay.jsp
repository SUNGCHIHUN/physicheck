...
<script type="text/javascript">
	...
	function getYearPerformance() {
		window.location='/admin/doctorPerformance?year=' + year;
	}
	...
	function selectYear() {
		...
		window.location='/admin/doctorPerformanceDay?year=' + year;
	}
			...
			<div style="padding-top: 50px; text-align: center">
				...
				<table class="table table-hover">
					...
					<c:forEach var="pd" items="${pdlist}" varStatus="status">
						<c:if test="${pd.examination != null}">
							<tr>
								<td align="center">
									<input type="hidden" name="date" value="${pd.performanceMonth}/${pd.performanceDay}">
									${pd.performanceMonth}월 ${pd.performanceDay}일
								</td>
								<td align="center">
									${pd.memberNo}
								</td>
								<td align="center">
									<input type="hidden" name="doctorName" value="${pd.name}">
									${pd.name}
								</td>
								<td align="center">
									${pd.gender}
								...
								<td align="center">
									<input type="hidden" name="examTotal" value="${pd.examTotal}">
									<fmt:formatNumber value="${pd.examTotal}" pattern="#,###원" />
								</td>
							</tr>
						</c:if>
					</c:forEach>

		...
		<div style="padding-top: 50px">
			<table class="table table-hover">
					...
					<c:forEach var="dto" items="${list}">
						<c:if test="${dto.basicNo != 0}">
						<tr>
							<td align="center">${dto.reserveNo}</td>
							<td align="center">${dto.memberNo}</td>
							<td align="center"><a href="basicResult?reserveNo=${dto.reserveNo}">${dto.name}</a></td>
					...
					<tr>
						...
						<c:if test="${paging.startPage > 10}">
							<a href="/doctor/resultList1?pageNum=${paging.prev}">[이전]</a>
						</c:if>
						
						<!-- 페이지번호 처리 -->
						<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
							<a href= "/doctor/resultList1?pageNum=${num}">${num}</a>
						</c:forEach>
						${num}
						<c:if test="${paging.endPage < paging.pageCount}">
							<a href="/doctor/resultList1?pageNum=${paging.next}">[다음]</a>
						</c:if>
						</td>
    			  </tr>
			</table>

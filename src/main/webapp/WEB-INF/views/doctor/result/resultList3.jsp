		...
		<div style="padding-top: 50px">
			<table class="table table-hover">
					...
					<c:forEach var="dto" items="${list}">
						<c:if test="${dto.mentalNo != 0}">
						<tr>
							<td align="center">${dto.reserveNo}</td>
							<td align="center">${dto.memberNo}</td>
							<td align="center"><a href="mentalResult?reserveNo=${dto.mentalNo}">${dto.name}</a></td>
					...
					<tr>
						...
						<c:if test="${paging.startPage > 10}">
							<a href="/doctor/resultList3?pageNum=${paging.prev}">[이전]</a>
						</c:if>
						
						<!-- 페이지번호 처리 -->
						<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
							<a href= "/doctor/resultList3?pageNum=${num}">${num}</a>
						</c:forEach>
						
						<c:if test="${paging.endPage < paging.pageCount}">
							<a href="/doctor/resultList3?pageNum=${paging.next}">[다음]</a>
						</c:if>
						</td>
    			  </tr>
			</table>
